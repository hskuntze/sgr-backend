...
public class IdentificacaoRiscoService {
	...
	public List<IdentificacaoRiscoDTO> getAll() {
		return identificacaoRiscoRepository.findAll().stream().map(ir -> new IdentificacaoRiscoDTO(ir)).collect(Collectors.toList());
	}
	
	public IdentificacaoRiscoDTO getById(String id) {
		IdentificacaoRisco ir = identificacaoRiscoRepository.findById(id).orElseThrow(() -> new RecursoNaoEncontradoException("Não foi possível encontrar uma IDENTIFICAÇÃO DE RISCO com ID " + id));
